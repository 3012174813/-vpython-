...
def 更新斜面(角度):
    ...
    斜面终点x = 斜面长度 * cos(斜面角度 * pi/180)
   
    if 斜面 is not None:
        斜面.visible = False
        
    斜面 = box(pos=vector(斜面终点x/2, 初始高度/2, 0), size=vector(斜面长度, 0.1, 20), axis=vector(cos(斜面角度 * pi/180), sin(斜面角度 * pi/180), 0), color=color.orange, opacity=0.5)
...
高度滑块 = slider(min=0, max=50, value=初始高度, length=200, bind=lambda s: 参数调整(s, "高度"), pos=场景.title_anchor,step=1)
速度滑块 = slider(min=0, max=20, value=初始水平速度, length=200, bind=lambda s: 参数调整(s, "速度"), pos=场景.title_anchor,step=1)
斜面滑块 = slider(min=0, max=90, value=斜面角度, length=200, bind=lambda s: 更新斜面(s.value), pos=场景.title_anchor,step=5)
...
while True:
    ...
    if 高度 > 0:
        ...
        当前角度 = atan(高度/(水平位置+0.001)) * 180/pi
